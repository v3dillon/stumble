...
pub(crate) fn definitions() -> &'static [ToolDefinition] {
    ...
    DEFINITIONS.get_or_init(|| vec![
        ...
        d(Tool::DeletePod, "delete_pod", CapabilityOnly(Capability::PodCuration), uuid_schema("pod_id"), Blocking, published(47, "Delete Pod", "Delete a locally owned Pod. The private Inbox cannot be deleted. A public Pod returns a Pending Proposal and issues a Pod Withdrawal when approved. Existing Subscriptions and copies on other nodes stay.", false, true)),
