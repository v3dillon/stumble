...
One Stumble-path clarification: when the Feed is caught up, the bare `stumble` command runs an empty-query `explore_public_pods` over already-synchronized announcements and best-effort Origin explore-sample fetches against up to 5 unsubscribed Origins. That path is index-free by design — it calls `explore_public_pods`, never the Index variant — so configured Index Nodes are still contacted only from explicit User-authored Explore.
